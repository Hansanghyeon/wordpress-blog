...
const Pagination = ({ basePath, per = 5, midSize = 5, current, total }: PaginationProps) => {
  ...
  const hasMorePrevStep = !(0 < current && current < midSize);
        ...
        <div className="justify-self-center">
          <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
            ...
            {!(0 < current && current < midSize) && (
              <>
                <Link href={url(1)} className="relative inline-flex items-center px-3 py-2 border text-sm font-medium btn-light dark:btn-dark rounded-l-md ">
            ...
            <Link href={r - 1 > 0 && url(r - 1)} disabled={hasMorePrev} className={clsx("relative inline-flex items-center px-2 py-2 border text-[14px] font-medium btn-light dark:btn-dark", { "rounded-l-md": !hasMorePrevStep })}>
            ...
            {/* NEXT Paignation */}
            <Link href={hasMoreNext && url(r + midSize)} disabled={!hasMoreNext} className={clsx("relative inline-flex items-center px-2 py-2 rounded-r-md border text-sm font-medium btn-light dark:btn-dark", { "rounded-r-md": !hasMoreNextStep })}>
            ...
            {!(last - midSize < current && current < last) && hasMoreNext && (
              <>
                <Link href={url(last)} className="relative inline-flex items-center px-3 py-2 border text-sm font-medium btn-light dark:btn-dark rounded-r-md ">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-[1em] w-[1em]" viewBox="0 0 20 20" fill="currentColor">
